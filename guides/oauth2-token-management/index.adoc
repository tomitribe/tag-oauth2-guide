...
configuring different aspects to the scheme. This allows a complete control of all the generated Toeken, Authentication
...
* _scope_ a list of roles assigned to the user in TAG.

== Validate a Token

Tokens may be invalid if they are beyond their expiration date or if they key used to sign it doesn't match the
OAuth 2.0 Security Profile of TAG.

TAG provides you a specific endpoint that you can use to validate if an `Access Token` is valid. To validate a token,
you need the `access_token` response value and post it in the `token` form parameter with the following `curl` command:
...
By default, TAG issues a Refresh Token that applications can use to retrieve a new Access Token when exprired. This
allows you to limit the risk of leaking an Access Token (depending of its expirations period). It also prevents the
need to resent the user credentails again since the new token is now obtained using the Refresh Token.
...
If you try to refresh with an old Refereh Token, you will get the following error:
...
TAG provides you a specific endpoint that you can use to revoke an `Access Token`. To revoke a token, you need the
